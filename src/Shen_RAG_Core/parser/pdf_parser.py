...
def _is_payment_error(error_str: str) -> bool:
    """Detect 402 Payment Required / out-of-credits errors — abort immediately."""
    return "402" in error_str or "Payment Required" in error_str or "insufficient" in error_str.lower()
    ...
    def _call_vision_api_with_retry(
        ...
        for attempt in range(self.max_retries_per_page):
            try:
                ...
                # 402 Payment Required — abort immediately, no retry
                if _is_payment_error(err_str):
                    logger.error(
                        "Payment/credits error on page %d of %s: %s. "
                        "Aborting vision extraction (use local fallback).",
                        page_number, doc_id, e,
                    )
                    return None
